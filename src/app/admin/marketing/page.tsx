...
import { createGlobalCoupon } from "@/features/admin/actions/marketing-actions"
...
export default async function AdminMarketingPage() {
  ...
  // Find coupons with no store attached
  ...
  return (
    <div className="space-y-8 max-w-4xl animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Manage global platform coupons and flash sales.</p>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-8">
        <div className="md:col-span-1">
          <form action={async (formData: FormData) => {
            "use server"
            const result = await createGlobalCoupon(formData)
            if (!result.success) console.error(result.error)
          }} className="space-y-4 bg-card rounded-2xl border border-border/40 p-6 shadow-sm sticky top-6">
            <h2 className="font-semibold mb-4">Create Global Coupon</h2>
            
            <div className="space-y-2">
              <label className="text-sm font-medium">Code</label>
              <input name="code" required className="w-full h-10 rounded-xl border border-input bg-background px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring uppercase" placeholder="WELCOME10" />
            </div>
